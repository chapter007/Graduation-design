	...
	var Pagination = function(element,options){
		...
		this.filling = function(index){
			...
			if(current >= opts.count + 2 && current != 1 && pageCount != opts.count){
				...
				//改了这里
				html += opts.coping ? '<a id="page1" href="./index.php?id='+id+'&page=1" data-page="1">'+home+'</a><span>...</span>' : '';
			}
			...
			for (;start <= end; start++) {
				if(start <= pageCount && start >= 1){
					if(start != current){
						//改了这里
						html += '<a id="page'+start+'" href="./index.php?id='+id+'&page='+start+'" data-page="'+start+'">'+ start +'</a>';
					}else{
